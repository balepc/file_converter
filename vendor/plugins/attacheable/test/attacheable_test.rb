...
class AttacheableTest < Test::Unit::TestCase
  ...
  def test_image_creation
    ...
    assert_equal "1_medium.jpg", image.send(:thumbnail_name_for, :medium), "should generate right thumbnail filename"
    assert_equal "1", image.attachment_basename
    assert_equal ".jpg", image.attachment_extname
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.jpg"), "File should be saved"
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_medium.jpg"), "Thumbnails should not be generated"
    image.destroy
  ...
  def test_file_renaming
    ...
    assert_equal "1", image.attachment_basename
    assert_equal ".jpg", image.attachment_extname
    assert_equal "/system/images/0000/0001/1_medium.jpg", image.public_filename(:medium)
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.jpg"), "File should be saved"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_medium.jpg"), "Thumbnails should be generated on demand"
    
    image = Image.find(1)
    image.filename = "non1.jpg"
    image.save
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.jpg"), "Old filename should not be kept"
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_medium.jpg"), "Thumbnails on renaming should be destroyd"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/non1.jpg"), "File should be saved"
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/non1_medium.jpg"), "Thumbnails on renaming should not be autocreated"
  ...
  def test_autocrop_image
    ...
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_preview.jpg"), "Thumbnails should be generated on demand"
    identify = `identify "#{File.dirname(__FILE__)+"/public/system/images/0000/0001/1_preview.jpg"}"`
  ...
  def test_with_sti
    ...
    assert image.save, "Image should be saved"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.jpg"), "File should be uploaded as for Image"
    image.destroy
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001"), "Directory should be cleaned"
  end
  ...
  def test_save_raw_binary
    ...
    image = Image.new(:uploaded_data => input)
    assert image.save, "Image should be saved"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.jpg"), "File should be uploaded"
  ...
  def test_save_msword
    ...
    image = Image.new(:uploaded_data => input)
    assert image.save, "Image should be saved"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.doc"), "File should be uploaded"
    assert !image.send(:full_filename_with_creation, :preview)
    assert_equal "/system/images/0000/0001/1.doc", image.public_filename
    image.destroy
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001"), "Directory should be cleaned"
  end
  ...
  def test_create_thumbnail
    ...
    assert_equal "1_medium.jpg", image.send(:thumbnail_name_for, :medium), "should generate right thumbnail filename"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.jpg"), "File should be saved"
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_medium.jpg"), "Thumbnails should not be autogenerated"
    photo, data = Image.data_by_path_info(%w(0000 0001 1_medium.jpg))
  ...
  def test_after_create_thumbnail
    ...
    assert photo, "Image should be guessed"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_medium.jpg"), "Thumbnails should be generated on demand"
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1_medium.jpg.backup"), "After callback should be called"
    assert data, "New file should be generated"
    image.destroy
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001"), "Directory should be cleaned"
  end
  ...
  def test_save_from_web
    ...
    assert File.exists?(File.dirname(__FILE__)+"/public/system/images/0000/0001/1.gif"), "file should be saved"
  end
  ...
  def test_string_upload
    image = Image.new(:uploaded_data => "1.jpg")
    assert image.save, "Image should be saved with empty file"
    assert !File.exists?(File.dirname(__FILE__)+"/public/system/images/0000"), "nothing should be created"
    assert image.destroy, "should be destroyable"
  end
